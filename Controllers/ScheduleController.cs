    ...
    public class ScheduleController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSchedule([FromQuery] string group)
        {
            try
            {
                ...
                HttpResponseMessage response = await client.GetAsync($"https://dekanat.lnu.edu.ua/cgi-bin/timetable_export.cgi?req_type=rozklad&req_mode=group&OBJ_ID=&OBJ_name={encodedGroup}&dep_name=&ros_text=united&begin_date={date.startDate}&end_date={date.endDate}&req_format=json&coding_mode=UTF8&bs=ok");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    scheduleModifier = scheduleModifier.Filter(schedule);
